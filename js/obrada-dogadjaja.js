...
$("#odabir-predmeta").submit(function(e) {
    ...
    conn.onclose = function(e) {
        conn.close();
        $("#pageloader").css("visibility", "hidden");
        if (e.code === 1000) {      // if the connection was closed normally after it was previously established
            ...
            $("#solution-navigation").hide();
            $("#error-message").html(tekst["websocketsConnectionsClosedAbnormally"]);
            $("#error-message").show();
        }
    };
});
...
function showBuildingLocationDefinitionWindow() {
    if (googleMapsApiKey === "") {
        alert(`${tekst['buildingLocationDefinitionDialog']['apiKeyRequiredForDistanceMatrixAutofill']}`);
        return;
    }
